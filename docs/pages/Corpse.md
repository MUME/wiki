---
title: Corpse
description: 'When a living creature dies, it leaves a corpse'
tags:
  - Help files
---
When a living creature dies, it leaves a [corpse](./Corpse.md)
which contains the worldly possessions that were carried at the moment
of [death](./Death.md). You can loot the corpses of your fallen
enemies and friends. You can also [bury](./Bury.md) corpses,
[burn](./Burn.md) them, or (if your mind is truly twisted) come up
with many other things you could do with the corpse of a fallen enemy.

If you have died, you can try to retrieve your
equipment, but be aware that corpses do decay
after some time. Rumour says this takes a bit longer than a full
MUME-day.

In case a true [hero](./Hero.md) meets his (not so final) demise,
there is a chance a guardian-[shadow](./Shadow.md) rises and tries
to prevent other players from looting the corpse.

When [drowning](./Drowning.md) in a river or other large body of
water, it may happen your corpse get carried away by the current. In
this case you may be able to discover it by following the stream of the
water.

Sometimes corpses can not be recovered at all, for example, if you
happened to die in an extremely dangerous place (also known as [death
trap](./Death_trap.md)).

Note that dense fog may make it impossible to see
items on the ground; that includes corpses.

Some creatures, such as [ghosts](./Ghost.md), do not leave a
corpse behind, once they are put to their final rest.

Corpses "inherit" the keywords of the corresponding living creature,
followed by "-corpse": for example, dwarf-corpse, wolf-corpse, and so
on.

See also: [Decapitate](./Decapitate.md), [Drag](./Drag.md),
[Drain](./Drain.md), [Hang](./Hang.md)

## External Links
- [Official MUME Help: Corpse](https://mume.org/help/corpse)
