---
title: Fangorn
aliases: ["Ancient oak", "Black alder", "Birch tree", "Haughty elm", "Mature linden tree", "Tall ash tree", "Tall Scots pine", "Slender hornbeam"]
description: 'Fangorn is the home of the Ents. It is a large forested area populated by trees like the tall Scots pine.'
tags:
  - Locations
---
**Fangorn** is the home of the [Ents](./Ent.md). It is a large
forested area, located between [Isengard](../lore.md#isengard) and
[Lorién](./Lothlórien.md), and is mainly populated by ents, huorns and
other [plants](./Plant.md). [Riding](./Ride.md) and
[scouting](./Scout.md) is not possible in most
[rooms](./Mechanics.md#room). [Drinking](./Items.md#drink) from the streams
and rivers in Fangorn grants a "refreshment" buff, the exact effects of
which are not known, though it seems to increase
[regeneration](./Regeneration.md). Beware when lighting
[campfires](./Camp.md) or [casting](./Cast.md) fire-based
[spells](./Spell.md) in the forest, as the ents do not like fire
and may be angered if they see you using it.

## Quests

- [An Ent and an axe](./Quest.md#an-ent-and-an-axe)
- [Beechbone's bowl](./Quest.md#beechbones-bowl)
- [Leaf-Cake](./Quest.md#leafcake)
- [Poisoned Forest](./Quest.md#poisoned-forest)
- [Harle and the Entwives](./Quest.md#harle-and-the-entwives)

## Mobiles

### Unique mobiles

- [Treebeard](./Treebeard.md)
- [Beechbone the Ent](./Beechbone_the_Ent.md)
- [Leaflock](./Fangorn.md#unique-mobiles)
- [Strange tree](./Strange_tree.md)

### Ents & huorns

Many varieties of [Ents](./Ent.md) and huorns inhabit the forest, including:
- **Ancient oak**: An ancient oak tree.
- **Black alder**: A black alder tree.
- **Birch tree**: A birch tree.
- **Haughty elm**: A haughty elm tree.
- [Imposing beech tree](./Imposing_beech_tree.md)
- **Mature linden tree**: A mature linden tree.
- **Tall ash tree**: A tall ash tree.
- **Tall Scots pine**: A tall Scots pine tree.
- **Slender hornbeam**: A slender hornbeam tree.

### Plants

- [Broomrape plant](./Broomrape_plant.md)
- [Brown fungus](./Brown_fungus.md)
- [Buckthorn](./Buckthorn.md)
- [Clump of roots](./Clump_of_roots.md)
- [Giant green fungus](./Giant_green_fungus.md)
- [Mass of vines](./Mass_of_vines.md)
- [Massive roots](./Fangorn.md#plants)
- [Rooting stems](./Rooting_stems.md)
- [Tangly shrub](./Tangly_shrub.md)
- [Thick tangle of vines](./Thick_tangle_of_vines.md)

### Other mobiles

- [Huge millipede](./Huge_millipede.md)
- [Small termite](./Small_termite.md)
- [Young mountain lion](./Young_mountain_lion.md)
- [Mountain lion](./Mountain_lion.md)
- [Mountain lioness](./Mountain_lioness.md)

<!--@include: ../includes/Place.md-->
