...
- Thank you to p Elestir, p Daki, p Razoor and p Wingfoot for help with
  the initial version of this guide. Anyone who wants, please feel free
  to directly change or add information
...
- Str:19 Int: 9 Wis: 7 Dex:15 Con:20 Wil: 11 Per:14.
  - Train neglect PER-2, train improve STR+1. When STR is trained to
    max, you can train WIS for better mixing.
  - You can easily go for envenom with these stats
  - Recommended % of pracs by p Wingfoot(one of the best pkers in MUME
    history):
    - Wingfoot: do not carry too many weapons like Conc+Smite+Slash, it
      decreases your DB because of weight and you get better pracs with
      just Conc. You can just do conc+slash and apply venom with slash
    - Endurance: max
    - Wilderness : max
    - Envenom: 61%
...
- Oil flask from floorboads e VT -\> goblet from packrats/PK
...
- Shield: Reinforced Oaken Shield(spirit s Ingrove) -\> Cardolan -\>
  Arthedain -\> Gilded -\> Leaf-embossed(DG Drake, need 4) /
  Bejewelled(need 4)
- BSB from WC(need 3)
- Sable pouch is just extra weight and doesn't protect you from fire
  spells, eff vs fire means protection against Bloodwight/Mithnaur
...
- Bows: longbow from hunter in bushes s of NOC -\> Embellished bow from
  PK
- Daggers: Barbed fang from commander on Moria / Sacrificial knife from
  Goblin Shaman/Brolg/Nagash -\> Nimble Blade from quest solo -\> BRD
  from Sage(need 5)/PK
- Arrows: war arrows from ABR/archer in egate Moria -\> Quest arrows
  from Narag/Steel-Tipped arrows from PK
- Purple scroll for kit upgrade from EP
- Skip black wrappings because it very likely has no effect on orcs
  ...
  vs light etc.
- Fine soft leather gloves: Luxzum peg, Cinard
- Full plain set(hood/shirt/sleeves/pants) in Lorien down Largestump
  inside chest (picks +1 needed to pick)
- Soft leather riding skirt(only pants that give +1ob, LOADS WHERE???)
- Archer's Wristband(solo)
- Opaque/Lambent amulet(need group of 5 to farm)
- Can do Tharbad for iron ring/notracks with huge group by luring to
  staircase

### Zorc Warrior re-eq progression

- Concussion: cruelly spiked from zaugurz officer/Vurgl -\> engraved
  warhammer from SK(need 3, scout can solo) -\> ornate(need 4 and
  obsidian eye needs to blind at least 2)
- Stabbing: giant spear from giant -\> awlpike from Dol Guldur(need 4)
...
- Full fine chain loads on Caravan in DG
- Fine chain coif: LBF(need 3), spirit soldiers in GC that load BW
  key(need 4)
- Fine chain leggings: Luxzum peg
- Fine chain hauberk: Goblin warrior
- Fine chain sleeves: Men before Magus(solo), LBF(need 3)
- Fine chain gloves: Guardian/DT captain room/Blood-drenched orc, Moria
  (part of patrol, the one that doesn't move, wws from armoury)(need 4)
- Black warg fur(very heavy, only for xp): Skeleton in DG(solo) / warg
  pack-leader, in the pine forest south of DT(need 3) -\> soot-black
  bear hide(lighter than bwf, still very heavy, only for xp) d
  greatboulder entrance to Emyn -\> mantle from PK
- Horned helmet from Crazed Dwarf w gate Moria
- Black-thorned wristband(need 5 to farm it)
...
- You get items to Narag, he gives you slender arrows, you cart them
...
- 4x Arrowheads in OIE: 1) 4s entrance 2) 3s entrance+u+w+all u with
  eagle 3) w n w silverpanel "Metal Works" 4) "Rampart's End" s e u n e
  e from entrance. Need to kill 2 vines and they entangle and hurt- skip
  in sun, in dark try sneak past, if entangled shoot it dead.

### Most important herblores

- Antidote(solo), healing(solo), venom(solo), arachnia(solo),
  heightened(DG Magus- need 4, ask Carl for help how to solo as scout-
  it's a secret strat with bats probably), dark-draught(Emyn-need 5 to
  farm, scout can solo)
- Use infusion from Hillmen woman to get rid of negative effect
...
- Zorcs can do DK, can't get star charm tho because Azra is aggro, give
  palantir to Saruman
- Zorcs can do dodge quest. For scarred fell beast need like 3
  warriors + 3 scouts, not worth, easier to butcher the beast that flies
  around Herbalist(just stand there in open for 5min and it might come)
- You can rest on Anduin with raft in inventory and you're supersafe
  even in day
- You can skip Anduin muddle by going 3w 6s 1u from Old Ford.
- Need minimum 7 prac climb to visit Kral. Zorcs can kill Kral and get
  0-2 zaugurz-draughts.
...
- If you give scalps to Bulgotha he sometimes gives you zaugurz-flask,
  you can give hillside letter to him for something as well.
